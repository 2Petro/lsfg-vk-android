...
// Per-frame stage logs are verbose (4 lines/frame); gate behind
// LSFG_NPU_VERBOSE=1. Verify + stats + errors always print.
static bool npuVerbose() {
    static int v = -1;
    if (v < 0)
        v = getenv("LSFG_NPU_VERBOSE") ? 1 : 0;
    return v != 0;
}
#define NVLOG(...)                      \
    do {                                \
...
static int nosyncCached = -1;

double Backend::dmaSync(int fd, int start) {
    if (nosyncCached < 0)
        nosyncCached = (getenv("LSFG_NPU_NOSYNC") || getenv("VKHOOK_NOSYNC")) ? 1 : 0;
    if (nosyncCached)
        return 0.0;
...
bool Backend::spawnWorker(const std::string& modelPath, const std::string& workerBin) {
    ...
    if (bin.empty()) {
        if (const char* e = getenv("LSFG_NPU_BIN"))
            bin = e;
        else if (const char* e2 = getenv("VKHOOK_NPU_BIN"))
            bin = e2;
    }
    if (model.empty()) {
        if (const char* e = getenv("LSFG_NPU_MODEL"))
            model = e;
        else if (const char* e2 = getenv("VKHOOK_NPU_MODEL"))
            model = e2;
    }
    if (bin.empty()) {
    ...
    if (stat(bin.c_str(), &st)) {
        nlog("worker binary missing: %s (set LSFG_NPU_BIN)", bin.c_str());
        return false;
    }
    if (stat(model.c_str(), &st)) {
        nlog("model missing: %s (set LSFG_NPU_MODEL)", model.c_str());
        return false;
    }
    int sv[2];
...
#undef RESOLVE
    ...
    nlog("staging memtype %u flags=0x%x", pickedType,
...
bool Backend::generate(VkImage curSwapImage, VkImage outImage,
    ...
    if (!ready_ || dead_)
        return false;
    if (getenv("LSFG_NPU_DRYRUN")) // passthrough: prove skeleton/delivery
        return false;
    {
        double t = nowMs();
    ...
    // Verify cadence: the FNV cross-check burns ~5ms of present-thread CPU
    // (1.4MB staging readback) plus a staging copy. Kept rare so it can't
    // knock FIFO acquire phase off a vblank beat (multi-33ms cascade).
    // LSFG_NPU_VERIFY_EVERY=N (default 240, 0 = off after proof).
    static long verifyEvery = -1;
    if (verifyEvery < 0) {
        verifyEvery = 240;
        if (const char* e = getenv("LSFG_NPU_VERIFY_EVERY"))
            verifyEvery = atol(e);
    }
    bool verifyThis =
        verifyEvery > 0 && ((runs_ + 1) % (uint64_t)verifyEvery == 0);
    ...
    if (getenv("LSFG_NPU_NORUN")) {
        NVLOG("frame %llu NORUN: skipping worker Run", (unsigned long long)k);
    } else if (!runWorker(req, npuMs, wfnv, hasFnv)) {
        dead_ = true;
