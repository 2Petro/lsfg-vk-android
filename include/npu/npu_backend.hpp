#pragma once
// NPU frame-generation backend for lsfg-vk (glibc/Turnip side only).
//
...
// RIFE on Hexagon HTP, GPU packs the interpolated frame back into lsfg's
// out_n image. Dataflow + sync mirror vk_npu_loop.cpp's proven feeder loop:
//
...
//
// Deliberately synchronous + lockstep (no private swapchain, no pacing
// thread): generate() runs on lsfg's present thread between its preCopy and
// postCopy, so lsfg's instance/device/swapchain handling, vsync pacing and
...
private:
    ...
    void release();
    static double nowMs();
    static double dmaSync(int fd, int start);
